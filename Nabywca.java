

public class Nabywca {
    
    private String imieNabywcy;
    private String nazwiskoNabywcy;
    private String adresNabywcy;
    private String NIPNabywcy;

   
    public String getImieNabywcy() {
        return imieNabywcy;
    }

  
    public void setImieNabywcy(String imieNabywcy) {
        this.imieNabywcy = imieNabywcy;
    }

  
    public String getNazwiskoNabywyc() {
        return nazwiskoNabywcy;
    }

  
    public void setNazwiskoNabywcy(String nazwiskoNabywcy) {
        this.nazwiskoNabywcy = nazwiskoNabywcy;
    }


    public String getAdresNabywcy() {
        return adresNabywcy;
    }

 
    public void setAdresNabywcy(String adresNabywcy) {
        this.adresNabywcy = adresNabywcy;
    }

    
    public String getNIPNabywcy() {
        return NIPNabywcy;
    }

    
    public void setNIPNabywcy(String NIPNabywcy) {
        this.NIPNabywcy = NIPNabywcy;
    }
    
}
